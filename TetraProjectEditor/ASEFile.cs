...
public class Cel: UserDataAcceptor {
    static readonly Color Opaque = Color.FromArgb(0,0,0,0);
					...
					case CHUNK_LAYER:
						{
							...
							if (layerType == 0  && !layer.layerName.StartsWith ("//")) {
...
//								}
					...
					case CHUNK_SLICE:
						{
							var slice = new Slice ();
							var keyCount = reader.ReadDWord ();
							var flags = reader.ReadDWord ();
							slice.has9Patches = (flags & 0x01) != 0;
							slice.hasPivot = (flags & 0x02) != 0;

							slice.reserved = reader.ReadDWord ();
							slice.name = reader.ReadUTF8 ();
							slice.keys = new Slice.Key[keyCount];
							for (int c = 0; c < keyCount; c++) {
								var key = new Slice.Key ();
								key.frameNumber = reader.ReadDWord (); 
								key.bounds = new Slice.Rect();
								key.bounds.x = reader.ReadDWord (); 
								key.bounds.y = reader.ReadDWord (); 
								key.bounds.w = reader.ReadDWord (); 
								key.bounds.h = reader.ReadDWord (); 
								if (slice.has9Patches) {
									key.center = new Slice.Rect();
									key.center.x = reader.ReadDWord(); 
									key.center.y = reader.ReadDWord(); 
									key.center.w = reader.ReadDWord(); 
									key.center.h = reader.ReadDWord(); 
								}
								if (slice.hasPivot) {
									key.pivot = new Slice.Vector2();
									key.pivot.x = reader.ReadDWord();
									key.pivot.y = reader.ReadDWord();
	...
	static Color[] ToColorBufferRGBA(byte[] bytes) {
	    ...
	    for (int i = 0; i < arr.Length; ++i) {
	        ...
	        color =  Color.FromArgb((int)(bytes[offset + 3]),(int)(bytes[offset]), (int)(bytes[offset + 1]), (int)(bytes[offset + 2]));
	        arr[i] = color;
	    }
